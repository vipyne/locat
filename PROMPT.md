...
iterations. The repository files, git history, `PLAN.md`, and `PROGRESS.md` are your ONLY shared
state. Read them before doing anything.

## Do this, in order

1. **Read `PLAN.md`** (the full approved spec) and **`PROGRESS.md`** (what's done so far).
   - If `PROGRESS.md` does not exist, create it: list every phase and its tasks from `PLAN.md` as
...
- **Never fake a verification.** If a step cannot be verified programmatically, say so explicitly
  in `PROGRESS.md` rather than checking it off.
- **Human-gated steps — stop, don't fake:** Some steps require a human and CANNOT be done
  ...
  `RALPH_BLOCKED.md`, create an empty file named `RALPH_BLOCKED`, commit, and stop. Do NOT
  pretend it passed.
...
- When **ALL non-human-gated phases are complete and verified**, create an empty file named
  `RALPH_DONE`, note the remaining human steps in `PROGRESS.md`, commit, and stop.

## Reference
- Spec / task source of truth: **`PLAN.md`**
